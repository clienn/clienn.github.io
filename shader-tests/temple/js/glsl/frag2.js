const fragCode = `#version 300 es
    ...
    void main()
    {
        ...
        //------------------------------------------
        #ifdef STATICCAM
            vec3 ocol = texelFetch( iChannel3, ivec2(gl_FragCoord.xy-0.5), 0 ).xyz;
            ...
            {
                if( abs(gl_FragCoord.x-2.5)<0.5 ) fragColor = vec4( ca[2], -dot(ca[2],ro) );
                if( abs(gl_FragCoord.x-1.5)<0.5 ) fragColor = vec4( ca[1], -dot(ca[1],ro) );
                if( abs(gl_FragCoord.x-0.5)<0.5 ) fragColor = vec4( ca[0], -dot(ca[0],ro) );
            }
            else
            {

...
function main(w, h) {
    ...
    // flagInfo.x = cardInfo.x;
    // flagInfo.y = h / 3 - flagInfo.h / 2;

    flagInfo.container.w *= scaleX;
    flagInfo.container.h *= scaleX;

    // flagInfo.container.x = cardInfo.x;
    // flagInfo.container.y = h / 3 - flagInfo.container.h / 2;
...
// function drawFlag() {
//     const { x, y, w, h, container } = flagInfo;
//     ctx.drawImage(AM.images.ph.img, x, y, w, h);
//     ctx.drawImage(AM.images.flagContainer.img, container.x, container.y, container.w, container.h);
// }
...
function getFlag() {
    let n = AM.flags.length;
    let rng = Math.floor(Math.random() * n);
    const { code } = AM.flags[rng];

    let correctRng = Math.floor(Math.random() * 2);
    let rng2 = correctRng ? rng :  Math.floor(Math.random() * n);

    const { capital, continent, name } = AM.flags[rng2];
...
function nextCard() {
    ...
    let nextFlag = getFlag();

    // cards[currCard].setText(phraseInfo.phrase);
    cards[currCard].flagCode = nextFlag.code;;
    cards[currCard].setText(nextFlag.name);
    answers[currCard] = nextFlag.result;
...
function addCard() {
    ...
    // let phraseInfo = getPhrase();
    let flag = getFlag();
    card.flagCode = flag.code;
    card.setText(flag.name);
    answers.push(flag.result);
    cards.push(card);
}

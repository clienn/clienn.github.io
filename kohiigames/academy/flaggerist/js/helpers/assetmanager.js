class AssetManager {
    ...
    initSVGAssets(path) {
        parseAssets(path, (data) => {
            ...
            for (let i = 0; i < data.length; ++i) {
                let url = 'assets/flags/' + data[i].flag;
                this.loadSVG(data[i].code, url, data[i].capital, data[i].code, data[i].continent, data[i].name, null);
            }
        })
    }
    ...
    loadSVG(id, url, capital, code, continent, name, callback) {
        this.images[id] = {
            img: new Image(),
